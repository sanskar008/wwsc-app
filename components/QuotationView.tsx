...
export default function QuotationView({ quotationId, onBack }: Props) {
  ...
  const handleDownloadPDF = async () => {
    ...
    setPdfLoading(true);
    try {
      await PDFGenerator.generateQuotationPDF(quotation, {
  ...
  if (!quotation) {
    return (
      <div className="text-center py-8">
        <p className="text-gray-600 dark:text-gray-300">Quotation not found.</p>
      </div>
    );
  }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="text-right">
            {quotation.referenceLetter && (
              <p className="text-sm text-gray-600 dark:text-gray-300">
                Ref: {quotation.referenceLetter}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          {quotation.toDesignation && (
            <p className="text-gray-700 dark:text-gray-300">
              {quotation.toDesignation}
            </p>
          )}
          {quotation.toDepartment && (
            <p className="text-gray-700 dark:text-gray-300">
              {quotation.toDepartment}
            </p>
          )}
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                  Rate including GST as per Unit Packing
                </th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                  Mfg By
                </th>
        ...
        <div className="flex justify-end mb-6">
          <div className="text-right">
            <div className="text-lg font-semibold text-gray-900 dark:text-white">
              Total: ₹{quotation.subtotal.toFixed(2)}
            </div>
          </div>
        </div>

        {quotation.notes && (
          <div className="mt-8 text-gray-700 dark:text-gray-300 whitespace-pre-line">
            {quotation.notes}
          </div>
        )}
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-top">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => PDFGenerator.exportQuotationToExcel(quotation)}
